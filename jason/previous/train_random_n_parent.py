...
from nParentDataset import ParentDataset
from tools import *
import argparse
import wandb
import os
...
wandb.init(project='In-Context-Learning', 
           entity='shaobowang', 
           name=f'Task2_random_bs{bs}_a{alpha}_T{T}_S{S}',
           config=vars(args)
        )
...
root_path = './data'
save_file_path = f'results/Task2_random/{bs}_{lr}_{alpha}_T{T}_S{S}'
...
model.layers[0].A.weight.data = torch.randn_like(model.layers[0].A.weight.data).to(device) * 0.01
model.layers[1].A.weight.data = torch.randn_like(model.layers[1].A.weight.data).to(device) * 0.01
...
dataset = ParentDataset(S, T, alpha, n_sample,k)
