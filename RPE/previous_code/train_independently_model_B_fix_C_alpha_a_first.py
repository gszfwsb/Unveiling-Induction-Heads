...
method_args = f'Formal_train_W_first_parent{n}_n{n_sample}_L{L}_S{S}_H{H}_lr1{lr1}_lr2{lr2}_opt{optim_method}_w+{w_plus}_w-{w_minus}_c_alpha_init{c_alpha_init}_a_init{a_init}_alpha{alpha}_n-epochs{n_epochs}'
root_path = './data'
save_file_path = f'results/{dataset}/{method_args}'
makedirs(save_file_path)

# Generate the TwoLayerCausalTransformer
model = TwoLayerTransformer(S, L, H, w_plus, w_minus, a_init, c_alpha_init, n_parent=n)
model.to(device)
...
visualize_W(W, H, L, n, save_file_path, 'init', phase=1, enable_wandb=enable_wandb)
...
for epoch in pbar:
    ...
    if epoch % eval_freq == 0:
        ...
        W = model.layer1.W.clone().cpu().detach().numpy()
        visualize_W(W, H, L,n,save_file_path, epoch, phase=1,enable_wandb=enable_wandb)
...
for epoch in pbar:
    ...
    for x, y in train_loader:
        ...
        if epoch % eval_freq == 0:
            ...
            visualize_C_alpha_grad(C_alpha_grad,  save_file_path, epoch, phase=2,enable_wandb=enable_wandb)
    ...
    if epoch % eval_freq == 0:
        C_alpha_list = model.layer2.C_alpha_list.data.clone().cpu().detach().numpy()[0]
        visualize_C_alpha(C_alpha_list,  dominating_C_alpha_value, dominating_C_alpha_index, save_file_path, epoch, phase=2,enable_wandb=enable_wandb)
...
visualize_W(W, H, L, n, save_file_path, 'end', phase=2,enable_wandb=enable_wandb)
