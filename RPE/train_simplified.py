...
def main():
    ...
    parser.add_argument('--low-degree',type=int, help='control the maximal degree of the polynomial kernel', default=3)
    ...
    if low_degree != -1:
        model = SimplifiedTwoLayerTransformer(
            S, L, H, w_plus, w_minus, a_init, c_alpha_init, n - 1, low_degree
        )
    else:
        model = SimplifiedTwoLayerTransformer(
            S, L, H, w_plus, w_minus, a_init, c_alpha_init, n - 1
        )
    model.to(device)
