...
class DisentangledTransformer(nn.Module):
    def __init__(self, S, n_heads, n_layers, T, d_out):
        ...
        self.output_layer = nn.Linear(self.dims[-1], d_out)
        # print(self.dims)
...
# Define the specific sequence [A, B, A, C, A] assuming A=0, B=1, C=2
specific_sequence = torch.tensor([0, 1, 0, 2, 0])
# One-hot encode this specific sequence
specific_sequence_one_hot = F.one_hot(specific_sequence, num_classes=S).float()
# Expand this sequence to match the batch size
specific_sequence_expanded = specific_sequence_one_hot.unsqueeze(0).expand(bs, -1, -1)
# Replace the beginning of each sequence in x with this one-hot encoded sequence
x[:, :specific_sequence.size(0), :] = specific_sequence_expanded
